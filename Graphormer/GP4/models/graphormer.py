...
from Graphormer.GP4.modules.graphormer_graph_encoder import GraphormerGraphEncoder
...
class GraphormerModel(nn.Module):
    def __init__(self, config):
        """
        Graphormer model with a configurable output size for [ex, prob] pairs.
        ...
        self.output_size = config.get("output_size", 100)  # 기본 출력 크기: 100
        self.num_pairs = 50  # [ex, prob] 형태의 쌍 개수
        ...
        # Output Layer: Embedding dimension -> [num_pairs, 2]
        self.output_layer = nn.Linear(config["embedding_dim"], self.num_pairs * 2)
    ...
    def forward(self, batched_data):
        ...
        Returns:
            Final output of the model in [ex, prob] pair format.
        """
        ...
        # Reshape output to [batch_size, num_pairs, 2]
        output = output.view(output.size(0), self.num_pairs, 2)
