...
def cross_validate_model(
    ...
    if TEST_DATASET == None:
        test_dataset = SMILESDataset(csv_file=dataset_path, attn_bias_w=1.0, target_type=target_type,
                                nominal_feature_vocab=nominal_dims, continuous_feature_names=continuous_feature_names,
                                global_cat_dim=global_cat_dim, global_cont_dim=global_cont_dim, is_global=True)
...
if __name__ == "__main__":
    ...
    # CSV 저장
    df.to_csv("CV_losses_result_SID.csv", index=False)
    print("Saved to .csv")
