...
def cross_validate_model(
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        criterion_1 = loss_fn_gen(loss_function_1)
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_loader:
                ...
                if loss_function_1 == "SID":
                    ...
                    loss_1 = torch.stack([
                        criterion_1(outputs[i].unsqueeze(0), targets[i].unsqueeze(0), mask[i], 1e-4)
                    ...
                    if SoftDTW_pulling == "No":
                        ...
                        loss_1 = torch.stack([
                            criterion_1(
                                outputs[i][:450].view(-1, 10).max(dim=1)[0].unsqueeze(0).unsqueeze(-1),# 10개씩 잘라서 최대값 취합 후 SoftDTW 적용
                ...
                if loss_function_2 == "SID":
                    mask = torch.ones_like(outputs, dtype=torch.bool)
                    loss_2 = torch.stack([
                        criterion_2(outputs[i].unsqueeze(0), targets[i].unsqueeze(0), mask[i], 1e-4)
                        for i in range(outputs.size(0))
                    ]).mean()
                ...
                weight = loss_modifier.compute_weights([loss_1, loss_2], model)  # ✅ GradNorm 적용
    ...
    final_results, best_model_path = train_model(
        ...
        patience = patience,
        SoftDTW_pulling=SoftDTW_pulling
    )
...
def evaluate_on_test_set(model, test_loader, device, target_type, loss_function_1, loss_function_2):
    results = {
