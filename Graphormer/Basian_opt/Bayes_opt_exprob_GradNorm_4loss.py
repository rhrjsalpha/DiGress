    ...
    def objective_function(**params):
        ...
        if CV:
            ...
            result = cross_validate_model(
                ...
                loss_function_ex_1=merged_params["loss_function_ex_1"],
                loss_function_ex_2=merged_params["loss_function_ex_2"],
                loss_function_prob_1=merged_params["loss_function_prob_1"],
                loss_function_prob_2=merged_params["loss_function_prob_2"],
        ...
        lossfn_ex_1 = merged_params["loss_function_ex_1"]
        lossfn_ex_2 = merged_params["loss_function_ex_2"]
        lossfn_prob_1 = merged_params["loss_function_prob_1"]
        lossfn_prob_2 = merged_params["loss_function_prob_2"]

        if iteration_counter >= init_points + n_iter:
            res_df = pd.DataFrame(results)
            res_df.to_csv(f"BayesianOptimization_default_exprob_{grid_count}"
                          f"_iter{iteration_counter}_{lossfn_ex_1}_{lossfn_ex_2}_{lossfn_prob_1}_{lossfn_prob_2}.csv")
        else:
            print("intermediate results save")
            res_df = pd.DataFrame(results)
            res_df.to_csv(
                f"BayesianOptimization_Intermediate_default_exprob_{grid_count}_{lossfn_ex_1}_{lossfn_ex_2}_{lossfn_prob_1}_{lossfn_prob_2}.csv")


        end_time = time.time()
        full_time = end_time-start_time
        print(
            f"▶ total_time {full_time},finish iteration {iteration_counter}/{init_points + n_iter}, fixed_param started {grid_count}/{total_grid_count}, "
            f"{lossfn_ex_1},{lossfn_ex_2},{lossfn_prob_1}, {lossfn_prob_2}")
...
fixed_param_grid = {
    ...
    "loss_function_ex_1": ["SoftDTW"],  # 'ex'에 대한 손실 함수 "SoftDTW", "MSE", "MAE", "Huber", "SID"
    "loss_function_ex_2": ["SID"],
    "loss_function_prob_1": ["SoftDTW"],  # 'prob'에 대한 손실 함수 "SoftDTW", "MSE", "MAE", "Huber", "SID"
...
dataset = SMILESDataset(csv_file="../graphormer_data/train_100.csv", attn_bias_w=1.0, target_type=fixed_param_grid["target_type"][0])
test_dataset = SMILESDataset(csv_file="../graphormer_data/train_100.csv", attn_bias_w=1.0, target_type=fixed_param_grid["target_type"][0])
...
final_results_df.to_csv(f"BayesianOptimization_exporb_eVOsc_4loss_{n_pair}.csv", index=False)