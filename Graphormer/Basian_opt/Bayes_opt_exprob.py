...
def bayesian_optimization_param_grid(
    ...
    def objective_function(**params):
        ...
        print(
            f"▶ total_time {full_time},finish iteration {iteration_counter}/{init_points + n_iter}, fixed_param started {grid_count}/{total_grid_count}, {lossfn_ex},{lossfn_prob}")
...
dataset = SMILESDataset(csv_file="../graphormer_data/train_50.csv", attn_bias_w=1.0, target_type=fixed_param_grid["target_type"][0])
test_dataset = SMILESDataset(csv_file="../graphormer_data/train_50.csv", attn_bias_w=1.0, target_type=fixed_param_grid["target_type"][0])
...
for combination in grid_dicts:
    ...
    fixed_params = create_fixed_params("../graphormer_data/train_50.csv", combination, dynamic=False)
...
final_results_df.to_csv("BayesianOptimization_exporb.csv", index=False)