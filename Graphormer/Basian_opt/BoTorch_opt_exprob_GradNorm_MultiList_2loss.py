...
def bayesian_optimization_botorch(param_bounds, fixed_params, dataset_path, testset_path, DATASET, TEST_DATASET,
    ...
    for _ in range(init_points):
        ...
        # 하이퍼파라미터 저장
        for idx, key in enumerate(param_bounds.keys()):
            value = sample[idx].item()
            if key in ["num_in_degree", "num_out_degree", "multi_hop_max_dist", "num_encoder_layers",
                ...
                result[key] = int(round(value))
                result["activation_fn_name"] = activation_fn_selector(result[key])
            else:
        ...
        for key in fixed_params.keys():
            if key not in result:
                ...
                result[key] = int(val) if isinstance(val, (int, float)) and str(val).isdigit() else val
    ...
    random_results_df.to_csv(f"botorch_intermediate_random_results_{name_loss_fn_e}_{name_loss_fn_p}_modellist.csv", index=False)
    ...
    for _ in range(n_iter):
        ...
        intermediate_results.to_csv(f"botorch_intermediate_results_{name_loss_fn_e}_{name_loss_fn_p}_modellist.csv", index=False)
