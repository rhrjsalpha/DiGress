...
from Graphormer.GP5.models.graphormer_CV_train_eVOsc_GradNorm_2loss_GPUs import cross_validate_model
import itertools
...
def run_ddp_training_external(args_dict, fold_idx):
    import json
    # args_dict를 json으로 저장
    json_path = f"./ddp_args_fold{fold_idx}.json"
    with open(json_path, "w") as f:
        json.dump(args_dict, f)

    # subprocess로 DDP 학습 실행
    command = f"python train_model_entrypoint.py --args {json_path}"
    subprocess.run(command, shell=True, check=True)

def CV_loss_fn_name_gen(loss_fn, ex_prob): ### CV_r2_ex
    if loss_fn == "SID":
        CV_col_name = f"val_sid_{ex_prob}_avg_CV"
    elif loss_fn == "SoftDTW":
        CV_col_name = f"val_softdtw_{ex_prob}_avg_CV"
    elif loss_fn == "MAE":
        CV_col_name = f"val_mae_{ex_prob}_avg_CV"
    elif loss_fn == "MSE":
        CV_col_name = f"val_rmse_{ex_prob}_avg_CV"
    else:
...
def objective_function(params, fixed_params, dataset_path, testset_path, DATASET, TEST_DATASET):
    ...
    result = cross_validate_model(
        ...
        loss_function_1=config_params["loss_function_ex"],
        loss_function_2=config_params["loss_function_prob"],
...
def bayesian_optimization_botorch(param_bounds, fixed_params, dataset_path, testset_path, DATASET, TEST_DATASET,
    ...
    for _ in range(init_points):
        ...
        for key in result1:
            print("res1",key, result1[key])
        result.update(result1)
        end_time = time.time()
        total_time = end_time - start_time
        result["total_time"] = total_time
        # ✅ random search 결과에 추가
        random_results.append(result1)

        print(f"init count: {counter}, total time: {total_time}")
    print("initial points search finished")
    random_results_df = pd.DataFrame(random_results)
    random_results_df.to_csv(f"botorch_intermediate_random_results_{name_loss_fn_e}_{name_loss_fn_p}.csv", index=False)
    ...
    for _ in range(n_iter):
        ...
        print(f"result at {counter}: {result}, total time: {total_time} ")
        results.append(result)

        # ✅ 중간 결과 저장 (매 iteration 마다)
        intermediate_results = pd.DataFrame(results)

        intermediate_results.to_csv(f"botorch_intermediate_results_{name_loss_fn_e}_{name_loss_fn_p}.csv", index=False)
...
if __name__ == "__main__":
    ...
    # ✅ 실행 코드
    fixed_params = create_fixed_params("../graphormer_data/train_50.csv", {}, dynamic=False)
    fixed_params["target_type"] = ["ex_prob"]  # "ex_prob"
    fixed_params["loss_function"] = ["MSE"]  # "SoftDTW", "MSE", "MAE", "Huber", "SID"
    fixed_params["loss_function_ex"] = ["SID"]
    fixed_params["loss_function_prob"] = ["SID"]
