...
def bayesian_optimization_botorch(param_bounds, fixed_params, dataset_path, testset_path, DATASET, TEST_DATASET,
    ...
    for counter in range(1, init_points + 1):
        ...
        print(f"initial points end {counter}/{init_points + 1}")
    ...
    for counter in range(1, n_iter + 1):
        ...
        pd.DataFrame(results).to_csv(f"botorch_multitask_intermediate_results_{name_loss_fn_e}_{name_loss_fn_p}.csv", index=False)
...
if __name__ == '__main__':
    ...
    # ✅ 실행 코드
    fixed_params = create_fixed_params("../graphormer_data/train_100.csv", {}, dynamic=False)
