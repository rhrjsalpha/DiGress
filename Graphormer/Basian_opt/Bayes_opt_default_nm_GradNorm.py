...
from Graphormer.GP5.models.Graphormer_train_default_GradNorm import train_model
from Graphormer.GP5.models.graphormer_CV_train_default_GradNorm import cross_validate_model
import time
    ...
    def objective_function(**params):
        ...
        if CV:
            ...
            result = cross_validate_model(
                ...
                loss_function_1=merged_params["loss_function_1"],
                loss_function_2=merged_params["loss_function_2"],
        ...
        lossfn_1=merged_params["loss_function_1"]
        lossfn_2=merged_params["loss_function_2"]
        if iteration_counter >= init_points + n_iter:
            res_df = pd.DataFrame(results)
            res_df.to_csv(f"BayesianOptimization_default_nm_grid{gird_count}_iter{iteration_counter}_{lossfn_1}_{lossfn_2}.csv")

        end_time = time.time()
        full_time = end_time-start_time
        print(
            f"▶ total_time {full_time},finish iteration {iteration_counter}/{init_points + n_iter}, fixed_param started {gird_count}/{total_grid_count}, {lossfn_1}_{lossfn_2}")
...
fixed_param_grid = {
    "edge_type": ["multi_hop"],
    #"activation_fn": ["gelu"],
    "loss_function_1": ["MSE", "MAE", "SID"], #"MSE", "MAE", "SoftDTW", "Huber", "SID"
    "loss_function_2": ["SoftDTW"],
    "SoftDTW_pulling":["Max"], #"No", "Max", "Mean"
    "pre_layernorm": [False],
    "q_noise": [0.0],
    "qn_block_size": [8],
    #"batch_size": [64],
...
for combination in grid_dicts:
    ...
    train = pd.read_csv("../graphormer_data/train_50.csv")
    test = pd.read_csv("../graphormer_data/test_100.csv")
...
final_results_df.to_csv("BayesianOptimization_default_nm_GradNorm.csv", index=False)