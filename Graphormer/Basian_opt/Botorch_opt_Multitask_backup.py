...
class MultiTaskHypervolumeAcquisition(MCAcquisitionFunction):
    def __init__(self, model, ref_point: Tensor, sampler, **kwargs):
        super().__init__(model=model, sampler=sampler, objective=IdentityMCMultiOutputObjective())
        self.ref_point = ref_point  # shape: (2,)
        self.device = ref_point.device

    def forward(self, X: Tensor) -> Tensor:
        if X.dim() == 3:
            X = X.squeeze(0)  # (1, q, d) -> (q, d)

        q = X.shape[-2]
        device = X.device
        task_feat_0 = torch.zeros(X.shape[0], q, 1, device=device)
        print(X.shape, task_feat_0.shape)

        # eV의 MAE (task 0)
        X_ev = torch.cat([X, task_feat_0], dim=-1)
        posterior_ev = self.model.posterior(X_ev)
        samples_ev = self.sampler(posterior_ev)[..., 0].unsqueeze(-1)  # eV의 MAE

        # Osc의 MSE (task 1)
        task_feat_1 = torch.ones(X.shape[0], q, 1, device=device)
        X_osc = torch.cat([X, task_feat_1], dim=-1)
        posterior_osc = self.model.posterior(X_osc)
        print("posterior_osc",posterior_osc)
        samples_osc = self.sampler(posterior_osc) # Osc의 MSE

        # Concatenate the selected outputs
        obj_samples = torch.cat([samples_ev, samples_osc], dim=-1)  # (mc_samples, q, 2)

        # Hypervolume 계산
        improvements = []
        hv = Hypervolume(self.ref_point)
        for i in range(obj_samples.shape[0]):
            pareto = obj_samples[i]  # (q, 2)
            if pareto.ndim == 3:
                pareto = pareto.squeeze(1)
            print(pareto)
            improvements.append(hv.compute(pareto))

        return torch.tensor(improvements, device=device).mean().unsqueeze(0)
...
def bayesian_optimization_botorch(param_bounds, fixed_params, dataset_path, testset_path, DATASET, TEST_DATASET,
    ...
    print("checkpoint1")
    pd.DataFrame(random_results).to_csv(f"botorch_intermediate_random_results_{name_loss_fn_e}_{name_loss_fn_p}.csv", index=False)
    ...
    print("train_Y_full",train_Y_full.shape)
    ...
    for counter in range(1, n_iter + 1):
        ...
        acq_func = MultiTaskHypervolumeAcquisition(
            model=model,
            ref_point=ref_point,
            sampler=sampler
        )
        ...
        pd.DataFrame(results).to_csv(f"botorch_intermediate_results_{name_loss_fn_e}_{name_loss_fn_p}.csv", index=False)
