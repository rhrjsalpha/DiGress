...
from Graphormer.GP5.models.graphormer_CV_train_eVOsc_GradNorm_4loss import cross_validate_model
import itertools
...
def objective_function(params, fixed_params, dataset_path, testset_path, DATASET, TEST_DATASET):
    ...
    result = cross_validate_model(
        ...
        loss_function_ex_1=config_params["loss_function_ex_1"],
        loss_function_ex_2=config_params["loss_function_ex_2"],
        loss_function_prob_1=config_params["loss_function_prob_1"],
        loss_function_prob_2=config_params["loss_function_prob_2"],
        num_epochs=config_params["epoch"],
    ...
    loss_fn_ex_1 = CV_loss_fn_name_gen(config_params["loss_function_ex_1"], "ex")
    loss_fn_ex_2 = CV_loss_fn_name_gen(config_params["loss_function_ex_2"], "ex")
    loss_fn_prob_1 = CV_loss_fn_name_gen(config_params["loss_function_prob_1"], "prob")
    loss_fn_prob_2 = CV_loss_fn_name_gen(config_params["loss_function_prob_2"], "prob")
    obj1 = torch.tensor(result.get(loss_fn_ex_1), dtype=torch.double)
    obj2 = torch.tensor(result.get(loss_fn_ex_2), dtype=torch.double)
    obj3 = torch.tensor(result.get(loss_fn_prob_1), dtype=torch.double)
    obj4 = torch.tensor(result.get(loss_fn_prob_2), dtype=torch.double)


    # ✅ CV 결과 반환
    result["obj1"] = obj1.item()
    result["obj2"] = obj2.item()
    result["obj3"] = obj3.item()
    result["obj4"] = obj4.item()
    return torch.stack([-obj1, -obj2, -obj3, -obj4], dim=-1), result
...
def bayesian_optimization_botorch(param_bounds, fixed_params, dataset_path, testset_path, DATASET, TEST_DATASET,
    ...
    results = []
    random_results = []
    name_loss_fn_e1 = fixed_params["loss_function_ex_1"]
    name_loss_fn_e2 = fixed_params["loss_function_ex_2"]
    name_loss_fn_p1 = fixed_params["loss_function_prob_1"]
    name_loss_fn_p2 = fixed_params["loss_function_prob_2"]
    ...
    for _ in range(init_points):
        ...
        if 'train_X' not in locals():
            ...
            train_Y_1 = obj[0].unsqueeze(0)
            train_Y_2 = obj[1].unsqueeze(0)
            train_Y_3 = obj[2].unsqueeze(0)
            train_Y_4 = obj[3].unsqueeze(0)
        else:
            train_X = torch.cat([train_X, sample.unsqueeze(0)], dim=0)
            train_Y_1 = torch.cat([train_Y_1, obj[0].unsqueeze(0)], dim=0)
            train_Y_2 = torch.cat([train_Y_2, obj[1].unsqueeze(0)], dim=0)
            train_Y_3 = torch.cat([train_Y_3, obj[2].unsqueeze(0)], dim=0)
            train_Y_4 = torch.cat([train_Y_4, obj[3].unsqueeze(0)], dim=0)

        if 'train_X' not in locals():
            train_X = sample.unsqueeze(0)
            train_Y_1 = obj[0].unsqueeze(0)
            train_Y_2 = obj[1].unsqueeze(0)
            train_Y_1 = obj[3].unsqueeze(0)
            train_Y_2 = obj[4].unsqueeze(0)
        else:
            train_X = torch.cat([train_X, sample.unsqueeze(0)], dim=0)
            train_Y_1 = torch.cat([train_Y_1, obj[0].unsqueeze(0)], dim=0)
            train_Y_2 = torch.cat([train_Y_2, obj[1].unsqueeze(0)], dim=0)
        ...
        result1 = {
            ...
            "best_value_3": train_Y_3.max().item(),
            "best_value_4": train_Y_4.max().item(),
            "hypervolume": None  # 초기 random search 단계에서는 hypervolume 없음
        }
        end_time = time.time()
        ...
        # ✅ random search 결과에 추가
        random_results.append(result)

        print(f"init count:{counter}, total_time:{total_time}")
    print("initial points search finished")
    random_results_df = pd.DataFrame(random_results)
    random_results_df.to_csv(f"botorch_intermediate_random_results_{name_loss_fn_e1}_{name_loss_fn_e2}_{name_loss_fn_p1}_{name_loss_fn_p2}.csv", index=False)
    ...
    model_3 = SingleTaskGP(train_X, train_Y_3.reshape(-1, 1), outcome_transform=Standardize(m=1))
    model_4 = SingleTaskGP(train_X, train_Y_4.reshape(-1, 1), outcome_transform=Standardize(m=1))

    # ✅ ModelListGP로 합침
    model = ModelListGP(model_1, model_2, model_3, model_4)
    ...
    mll_4 = ExactMarginalLogLikelihood(model_4.likelihood, model_4)

    fit_gpytorch_mll(mll_1)
    fit_gpytorch_mll(mll_2)
    fit_gpytorch_mll(mll_3)
    fit_gpytorch_mll(mll_4)

    ref_point = torch.tensor([train_Y_1.min().item(), train_Y_2.min().item(), train_Y_3.min().item(), train_Y_4.min().item()]) - 0.1 #최소값에서 0.1 을 빼서 그곳을 원점으로 사용함
    ...
    for _ in range(n_iter):
        ...
        new_y_1, new_y_2, new_y_3, new_y_4 = obj[0], obj[1], obj[2], obj[3]

        train_Y_1 = torch.cat([train_Y_1, new_y_1.reshape(-1)], dim=0)
        train_Y_2 = torch.cat([train_Y_2, new_y_2.reshape(-1)], dim=0)
        train_Y_3 = torch.cat([train_Y_3, new_y_3.reshape(-1)], dim=0)
        train_Y_4 = torch.cat([train_Y_4, new_y_4.reshape(-1)], dim=0)
        train_X = torch.cat([train_X, new_x])

        model_1.set_train_data(train_X, train_Y_1, strict=False)
        model_2.set_train_data(train_X, train_Y_2, strict=False)
        model_2.set_train_data(train_X, train_Y_3, strict=False)
        model_2.set_train_data(train_X, train_Y_4, strict=False)

        fit_gpytorch_mll(mll_1)
        fit_gpytorch_mll(mll_2)

        # 하이퍼볼륨 계산
        pareto_mask = is_non_dominated(torch.cat([train_Y_1.unsqueeze(-1), train_Y_2.unsqueeze(-1), train_Y_3.unsqueeze(-1), train_Y_4.unsqueeze(-1)], dim=-1))
        pareto_front = torch.cat([train_Y_1.unsqueeze(-1), train_Y_2.unsqueeze(-1), train_Y_3.unsqueeze(-1), train_Y_4.unsqueeze(-1)], dim=-1)[pareto_mask]
        ...
        intermediate_results.to_csv(f"botorch_intermediate_results_{name_loss_fn_e1}_{name_loss_fn_e2}_{name_loss_fn_p1}_{name_loss_fn_p2}.csv", index=False)
...
# ✅ 실행 코드
fixed_params = create_fixed_params("../graphormer_data/train_1000.csv", {}, dynamic=False)
fixed_params["target_type"] = ["ex_prob"] # "ex_prob"
fixed_params["loss_function"] = ["MSE"] # "SoftDTW", "MSE", "MAE", "Huber", "SID"
fixed_params["loss_function_ex_1"] = ["SoftDTW"]
fixed_params["loss_function_ex_2"] = ["SID"]
fixed_params["loss_function_prob_1"] = ["SoftDTW"]
fixed_params["loss_function_prob_2"] = ["SID"]
...
dataset = SMILESDataset(csv_file="../graphormer_data/train_1000.csv", attn_bias_w=1.0, target_type="ex_prob")
test_dataset = SMILESDataset(csv_file="../graphormer_data/test_100.csv", attn_bias_w=1.0, target_type="ex_prob")
...
results = bayesian_optimization_botorch(
    ...
    testset_path="../graphormer_data/test_100.csv",
    DATASET=dataset,
    TEST_DATASET=test_dataset,
    init_points=10,
    n_iter=30
