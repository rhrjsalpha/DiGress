...
def collate_fn(batch, dataset, n_pairs=None): # DataLoader에서 collatefn 에 의해 SMILESDataset의 __getitem__에 의해 불려온 것들의 처리가 일어남, 이때 idx는 기존 전체의 idx 기
    """
    ...
    elif dataset.target_type == "default" or dataset.target_type == "nm_distribution":
        targets = torch.stack([target for _, target, _ in batch])
        #print("default target.size",targets.shape)
        targets = QM2D_to_1D(targets, eV_interval=0.01)
        #print("default target.size 2", targets.shape)
        targets = targets.T
    else:
        print("Target type error in collate_fn")
        targets = torch.stack([target for _, target, _ in batch])
    #print(f"Using batch index: {batch_indices}")
