...
# (2) YAML 설정 경로 (csv_spec.yaml) ★★ 여기도 확인
...
NUM_MOLS_PER_CONDITION = 50
# (6) VRAM 고려해서 한 번에 sample_batch 에 넣을 최대 batch size
MAX_BATCH_SIZE = 16
...
def load_trained_model(ckpt_path: str, device: torch.device) -> DiscreteDenoisingDiffusion:
    ...
    print("len(model.node_dist.prob) =", len(model.node_dist.prob))
...
def sample_molecules_with_conditions(
    ...
    """
    (예전 단일 batch 버전 – 현재는 MAX_BATCH_SIZE 로 잘라 쓰는 batched 버전을 사용 권장)
...
# ------------------------------------------------------------------
# 6. (새 버전) cond_y 전체를 펼친 뒤 MAX_BATCH_SIZE 로만 나눠서 샘플링
# ------------------------------------------------------------------
def sample_molecules_with_conditions_batched(
    model: DiscreteDenoisingDiffusion,
    ...
    """
    chunk_cond_size 같은 2단계 청크는 없애고,
    cond_y_batch 전체를 한 번에 펼친 후,
    max_batch_size 단위로 잘라서 sample_batch 를 반복 호출.

    - cond_y_batch: (num_conds, y_dim)
    - 각 조건당 num_mols_per_condition 개씩 반복
    - 전체 개수 = num_conds * num_mols_per_condition
    - VRAM 은 max_batch_size 로만 제어
    """
    device = next(model.parameters()).device
    num_conds = cond_y_batch.size(0)

    # (num_conds * num_mols_per_condition, y_dim)
    full_batch = cond_y_batch.repeat_interleave(num_mols_per_condition, dim=0)
    total = full_batch.size(0)

    number_chain_steps = int(model.T - 1)

    # 전체 batch 를 max_batch_size 로 나눔
    ...
    print(f"[INFO] Sampling in {num_batches} batches (max_batch_size={max_batch_size})")

    all_molecules = []

    # tqdm 로 batch 진행 표시
    for b_idx in tqdm.tqdm(range(num_batches), desc="Sampling batches", unit="batch"):
        start = b_idx * max_batch_size
        end   = min(total, (b_idx + 1) * max_batch_size)
        cond_sub = full_batch[start:end]    # [B_sub, y_dim]

        with torch.no_grad():
            molecules = model.sample_batch(
                batch_id=start,                    # 대충 인덱스로만 사용
    ...
    print(f"[INFO] Sampled total {len(all_molecules)} graphs.")
...
def save_smiles_with_conditions(
    ...
    condY_manual_from_yaml.csv (cond_df)와 매칭해서

    - PNG: cond{cond_id}_rep{k}.png
        ...
        for cond_idx in range(num_conds):
            ...
            for rep in range(num_mols_per_condition):
                ...
                row_out = {
                    ...
                    "rep_idx": rep,
                    "SMILES": smi,
                    "center_nm": row_cond.get("center_nm", None),
                    "sigma_nm": row_cond.get("sigma_nm", None),
                    "height": row_cond.get("height", None),
                    "pH_label": row_cond.get("pH_label", None),
                    "dielectric_constant_avg": row_cond.get("dielectric_constant_avg", None),
                    "is_qm": row_cond.get("is_qm", None),
    ...
    print(f"[INFO] SMILES + condY 를 {csv_path} 에 저장했습니다.")
...
def main():
    ...
    while True:
        ...
        if nm > 800:
            break
        center_nm_list.append(nm)
    print("center_nm_list", center_nm_list)
    #center_nm_list = [250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750]

    sigma_nm_list  = [10.0, 20.0, 30.0, 40, 50]

    base_pH_label            = "neutral"
    base_dielectric_constant = [1.0, 80.1]
    base_is_qm               = 0
    base_type_label          = "EM"
    ...
    # (3) condY CSV 및 텐서 로드
    cond_df = pd.read_csv(CONDY_CSV_PATH)
    ...
    # (5) 조건부 샘플링 (chunk 없이, MAX_BATCH_SIZE 로만 나눠서)
    molecule_list = sample_molecules_with_conditions_batched(
        model=model,
